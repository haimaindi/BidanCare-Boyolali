...
import { StokBerjalanBhp } from "../data/dummy";
...
import { fetchBhpJurnalBySku } from "../../../logic/services/manajemenBhpService.js";

interface StokBerjalanBhpTableProps {
  data: StokBerjalanBhp[];
  setData: React.Dispatch<React.SetStateAction<StokBerjalanBhp[]>>;
  onDelete?: (sku: string) => Promise<void> | void;
}

export function StokBerjalanBhpTable({ data, setData, onDelete }: StokBerjalanBhpTableProps) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<StokBerjalanBhp | null>(null);
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    fetchBhpJurnalBySku(activeSkuDetail, jurnalPage, JURNAL_PAGE_SIZE).then((res) => {
      if (isMounted) {
        setJurnalData(res);
      }
    });
          ...
          newItem.jurnal = [
            {
              ...
              keterangan: "Perbaikan data stok BHP (Manual)"
            },
            ...item.jurnal
          ];
        }
    ...
    return (
      <div className="space-y-[1.5rem]">
        <div className="flex items-center gap-[1rem]">
          ...
          <div>
            <h3 className={cn(tokens.typography.h3, "text-gray-900")}>Jurnal Stok BHP - {detailItem.namaBhp}</h3>
            <p className="text-sm text-gray-500">SKU: {detailItem.sku} | Kategori: {detailItem.kategori}</p>
          </div>
        </div>
    ...
    <>
      <Card className="h-full">
        <CardContent className="pt-[1.5rem]">
          ...
          {/* Pagination Table Stok Berjalan BHP Agnostik */}
          {data.length > 0 && (
            <div className="flex items-center justify-between border-t border-gray-100 pt-[1rem] mt-[1rem] text-sm text-gray-600">
              <div>
      ...
      >
        <div className="flex flex-col items-center text-center p-[0.5rem]">
          ...
          <p className="text-sm text-gray-500">
            BHP <span className="font-semibold text-gray-800">{deleteTarget?.namaBhp}</span> (SKU: {deleteTarget?.sku}) akan dihapus. Seluruh data terkait pada tabel <span className="font-medium text-rose-600">BHP masuk, BHP keluar, dan jurnal stok</span> juga akan terhapus secara permanen.
          </p>
        </div>
      </PopUpModal>
    </>
